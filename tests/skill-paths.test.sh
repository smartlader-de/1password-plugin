...
[ -d "$package_root/skills/cli-auth" ] || { echo "FAIL: skills/cli-auth/ missing"; exit 1; }
[ -f "$package_root/skills/cli-auth/SKILL.md" ] || { echo "FAIL: skills/cli-auth/SKILL.md missing"; exit 1; }
[ -f "$package_root/references/vercel.md" ] || { echo "FAIL: references/vercel.md missing"; exit 1; }

for ref in docs-map items-vaults secret-references service-accounts-connect sdks environments-transfer shell-plugins local-env-mount provider-sync ai-access; do
